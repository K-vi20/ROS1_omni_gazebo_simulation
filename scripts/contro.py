...
รองรับ 8 ทิศทาง, ปรับความเร็วได้ (Arrow Keys + Fine Adjust), Safety Timeout
"""
import sys
import tty
import termios
...
SPEED_STEP = 0.1          # Normal adjustment step
FINE_STEP = 0.01          # Fine adjustment step (10x smaller)
...
║    w   q   e              j (CCW)      ↑ / ↓ (Linear ±0.1)     ║
║  a       d                l (CW)       ← / → (Angular ±0.1)    ║
║    z   c                                                         ║
║                                                                  ║
║  Fine Adjust:  Shift+r/f (Linear ±0.01)  Shift+t/g (Ang ±0.01) ║
║  Quick Presets:  [1]=0.1  [2]=0.3  [3]=0.5  [4]=1.0 m/s        ║
║                                                                  ║
...
def get_key_nonblocking():
    """อ่านค่าปุ่มแบบ Non-blocking รองรับ Arrow Keys"""
    dr, _, _ = select.select([sys.stdin], [], [], 0)
    if dr:
        char = sys.stdin.read(1)
        # ตรวจสอบว่าเป็น Escape sequence (Arrow keys) หรือไม่
        if char == '\x1b':
            # อ่านอีก 2 bytes สำหรับ arrow key
...
def main():
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            if key:
                ...
                if key == ' ':     # Emergency Stop
                    ...
                    key_states[key] = current_time  # อัปเดตเวลากดล่าสุด
            ...
            # 3. ปรับความเร็ว (Speed Adjustment) - Arrow Keys
            if 'UP' in active_keys and LIN_SPEED < MAX_LIN_SPEED:
                LIN_SPEED = clamp(LIN_SPEED + SPEED_STEP, MIN_LIN_SPEED, MAX_LIN_SPEED)
                key_states.pop('UP', None)
            if 'DOWN' in active_keys and LIN_SPEED > MIN_LIN_SPEED:
                LIN_SPEED = clamp(LIN_SPEED - SPEED_STEP, MIN_LIN_SPEED, MAX_LIN_SPEED)
                key_states.pop('DOWN', None)
            if 'RIGHT' in active_keys and ANG_SPEED < MAX_ANG_SPEED:
                ANG_SPEED = clamp(ANG_SPEED + SPEED_STEP, MIN_ANG_SPEED, MAX_ANG_SPEED)
                key_states.pop('RIGHT', None)
            if 'LEFT' in active_keys and ANG_SPEED > MIN_ANG_SPEED:
                ANG_SPEED = clamp(ANG_SPEED - SPEED_STEP, MIN_ANG_SPEED, MAX_ANG_SPEED)
                key_states.pop('LEFT', None)
            
            # 4. Fine Adjustment (ปรับละเอียด) - Shift + keys
            if 'R' in active_keys and LIN_SPEED < MAX_LIN_SPEED:  # Shift+R
                LIN_SPEED = clamp(LIN_SPEED + FINE_STEP, MIN_LIN_SPEED, MAX_LIN_SPEED)
                key_states.pop('R', None)
            if 'F' in active_keys and LIN_SPEED > MIN_LIN_SPEED:  # Shift+F
                LIN_SPEED = clamp(LIN_SPEED - FINE_STEP, MIN_LIN_SPEED, MAX_LIN_SPEED)
                key_states.pop('F', None)
            if 'T' in active_keys and ANG_SPEED < MAX_ANG_SPEED:  # Shift+T
                ANG_SPEED = clamp(ANG_SPEED + FINE_STEP, MIN_ANG_SPEED, MAX_ANG_SPEED)
                key_states.pop('T', None)
            if 'G' in active_keys and ANG_SPEED > MIN_ANG_SPEED:  # Shift+G
                ANG_SPEED = clamp(ANG_SPEED - FINE_STEP, MIN_ANG_SPEED, MAX_ANG_SPEED)
                key_states.pop('G', None)
            
            # 5. Normal Adjustment (r/f, t/g)
            ...
            if '1' in active_keys:
                LIN_SPEED = 0.1
                key_states.pop('1', None)
            if '2' in active_keys:
                LIN_SPEED = 0.3
                key_states.pop('2', None)
            if '3' in active_keys:
                LIN_SPEED = 0.5
                key_states.pop('3', None)
            if '4' in active_keys:
                LIN_SPEED = 1.0
                key_states.pop('4', None)

            # 7. คำนวณเวกเตอร์การเคลื่อนที่ (Kinematics)
            ...
            # 8. Vector Normalization (สำคัญมากสำหรับ Control Stability)
        ...
        # Safety Failsafe: คืนค่า Terminal และสั่งหยุดหุ่นยนต์เสมอ
